...
function viewSkill(id) {
  ...
  if (s.real) {
    realSection = '<div class="sec"><div class="sec-t">🔗 来源与安装（真实开源 Skill）</div>' +
      ...
      '<span class="lineage">平台运行时：装载后 SKILL.md 全文注入陪跑 Agent 的 system prompt（接入点 js/api.js → WowAPI.runChat）；工作流/判断/边界均提取自源仓库原文。</span>' +
      '</div></div>';
  }
  ...
  return '<div class="view">' +
    ...
    combosHtml + variantsHtml + realSection +
...
function viewRun() {
  ...
  var feedHtml = run.feed.map(function (f) {
    return '<div class="agent-msg' + (f.warn ? ' warn' : '') + (f.user ? ' user' : '') + '">' +
      '<div class="am-t">' + esc(f.t) + '</div>' + nl2br(f.c) + '</div>';
  }).join('');
...
function bindRun(root) {
  ...
  function sendChat() {
    var inp = $('#run-input', root); var v = inp.value.trim(); if (!v) return;
    inp.value = '';
    ...
    WowAPI.runChat(v, { skillId: run.skillId, day: run.day, runId: run.id, execId: run.execId }).then(function (res) {
      ...
      if (res.boundaryHit) toast('边界停机已触发 · 这是纪律，不是功能');
    });
  }
...
function openFinishFlow(run) {
  var s = DB.skills[run.skillId];
  var q1 = run.id === 'r1'
    ? ['第 3 题卡住的时候', '当记分员看懂赛制的那一刻']
    : ['做这件事的某个瞬间', '和人交流的某个瞬间'];
  var q2 = run.id === 'r1'
    ? ['进门自我介绍那一段', '没有想逃的时刻']
    : ['开始前的犹豫', '没有想逃的时刻'];
  function chipRow(id, opts) {
    return '<div class="hero-chips" id="' + id + '" style="margin:6px 0 0">' +
      opts.map(function (o) { return '<button class="chip-btn">' + esc(o) + '</button>'; }).join('') + '</div>';
  }
  openModal(
    '<span class="card-kind">收尾只有两个问题 + 一句 verdict</span>' +
    '<h2 style="font-size:19px;font-weight:900">「' + esc(s.title) + '」结束了，不打分</h2>' +
    '<div class="sec"><div class="sec-t">① 哪个时刻你忘了时间？</div>' + chipRow('fq1', q1) + '</div>' +
    '<div class="sec"><div class="sec-t">② 哪个时刻你想逃？</div>' + chipRow('fq2', q2) + '</div>' +
    '<div class="sec"><div class="sec-t">③ 这个 Skill 在你的情况下成立吗？</div>' + chipRow('fq3', ['成立 ✓', '不成立（补一句反例）']) + '</div>' +
  ...
  ['fq1', 'fq2', 'fq3'].forEach(function (qid) {
    ...
    $all('.chip-btn', box).forEach(function (b) {
      b.addEventListener('click', function () {
        ...
        if (!box.dataset.done) { box.dataset.done = '1'; picked++; }
        if (picked === 3) afterVerdict(run);
      });
    });
  });
}
